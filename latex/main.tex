...
Ce rapport présente le projet AMIgo, une initiative stratégique d'unification des données client pour AMI Paris, marque de mode parisienne en pleine expansion internationale. Dans un secteur du luxe où l'expérience client personnalisée est devenue un différenciateur essentiel, AMI Paris faisait face à une fragmentation critique de ses données clients entre son système de gestion de boutiques physiques (Cegid Y2) et sa plateforme e-commerce (Shopify).

Le cœur du projet consiste en la conception et l'implémentation d'un Référentiel Client Unifié (RCU) sur Salesforce qui sert désormais de source unique de vérité pour toutes les interactions client. Cette architecture d'intégration permet non seulement la réconciliation des identités clients cross-canal, mais aussi la synchronisation bidirectionnelle en temps réel des données entre les systèmes.

Notre approche technique s'est articulée autour de trois axes majeurs :

\begin{enumerate}
    \item \textbf{Harmonisation du modèle de données} — Création d'une structure unifiée avec des règles métier pour la priorisation des sources et la validation des données
    \item \textbf{Système d'intégration robuste} — Développement d'une architecture de webhooks et de transformation de payloads configurable via métadonnées plutôt que par code
    \item \textbf{Mécanismes avancés de gouvernance} — Mise en place de processus de déduplication avec scoring de similarité et traçabilité complète des fusions
\end{enumerate}

Le projet a livré des résultats tangibles, notamment :
\begin{itemize}\itemsep0em
  \item Une réduction de 84\% du temps consacré à la gestion manuelle des doublons
  \item Une augmentation de 22\% de la valeur client moyenne calculée grâce à la consolidation des historiques d'achat
  \item Une amélioration de 26\% du taux de contacts email réussis
  \item Une hausse de 28\% du taux d'identification des clients en boutique
\end{itemize}
