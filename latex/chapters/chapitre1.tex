% Chapitre 1: Contexte Organisationnel et Technique

\chapter{Contexte Organisationnel et Technique}

% Encadré d'introduction
\begin{mdframed}[style=breakablegreen]
Ce chapitre présente le contexte organisationnel et technique du projet AMIgo, en détaillant l'écosystème des entreprises impliquées, l'état initial des systèmes d'information, les enjeux stratégiques et la méthodologie adoptée.
\end{mdframed}

\section{Présentation de Reetain Consulting}

\begin{center}
    \includegraphics[width=0.25\textwidth]{../images/logos/logo_reetain.png} \\
    {\itshape - Logo de Reetain -}
\end{center}

Reetain est une agence digitale créée en 2020 et spécialisée dans le conseil en CRM. Reetain est une équipe d'experts sur la plateforme Salesforce Marketing Cloud basée à Paris et à Casablanca.

\begin{mdframed}[backgroundcolor=notegreen, linewidth=0pt, innerleftmargin=10pt, innerrightmargin=10pt]
\textbf{Mission de Reetain :} Accompagner les clients dans l'optimisation de leur pratique CRM, la gestion de leurs campagnes marketing et l'amélioration de leur efficacité opérationnelle.
\end{mdframed}

Reetain Consulting renforce les opérations de campagne de ses clients et offre un service où il est possible de :

\begin{itemize}
    \item Faciliter la gestion de leurs opérations quotidiennes afin qu'ils puissent se concentrer sur leurs défis commerciaux les plus importants
    \item Augmenter leur capacité interne pendant les périodes de pointe pour gérer la charge de travail supplémentaire
    \item Être habilité par une équipe experte dans Salesforce Marketing Cloud
\end{itemize}

\subsection{Expertise et Services}

Les domaines d'expertise de Reetain Consulting s'articulent autour de quatre axes principaux:

\begin{itemize}
    \item \textbf{Design} : Élaborer des cas d'utilisation basés sur les priorités du client et développer des produits conformément à une feuille de route.
    \item \textbf{Deliver} : Fournir des fonctionnalités prêtes à l'emploi sur Salesforce Marketing Cloud.
    \item \textbf{Operate} : Exécuter vos campagnes marketing de manière habile et efficace.
    \item \textbf{Adopt} : Offrir des formations utiles et promouvoir les meilleures pratiques.
\end{itemize}

\subsection{Feuille de Route du Produit Marketing Cloud}

Convertir la vision de l'entreprise en capacités concrètes peut être un défi, notamment lorsqu'il s'agit de créer des parcours clients personnalisés. Le client de Reetain avait besoin d'une expertise solide pour optimiser ses capacités d'engagement client et tirer pleinement parti de la plateforme. Pour y parvenir, Reetain a aidé le client à construire une feuille de route exécutable depuis l'état actuel jusqu'à l'état futur, en suivant une approche par étapes :

\begin{enumerate}
    \item Évaluation de la plateforme
    \item Cartographie des capacités et analyse des écarts
    \item Priorisation basée sur la valeur commerciale et la complexité
    \item Exécution de la feuille de route en se concentrant sur l'omni-canal, l'IA, la performance et la pression commerciale.
\end{enumerate}

\subsection{Opérations de Campagne Marketing}

Les clients recherchent une combinaison unique de compétences alliant une expérience approfondie, une compétence opérationnelle et une attitude entrepreneuriale pour exécuter leurs campagnes avec Salesforce Marketing Cloud. Le client de Reetain devait lancer des publicités de la plus haute qualité avec un délai de mise sur le marché rapide. Pour ce faire, il était crucial de gérer toutes les activités du cycle de vie des campagnes tout en rationalisant les opérations :

\begin{itemize}
    \item[A.] Création et validation du brief
    \item[B.] Gestion des données et ciblage
    \item[C.] Création de contenu
    \item[D.] Gestion des tests et des versions
    \item[E.] Reporting et mesure de la performance
\end{itemize}
\newpage
\section{Concepts clés}


Cette section présente les concepts fondamentaux nécessaires à la compréhension du projet AMIgo.

\subsection{CRM (Customer Relationship Management)}

Le CRM, ou gestion de la relation client, est l'art de maximiser les relations d'une entreprise avec ses clients actuels et potentiels. Les technologies CRM facilitent la concentration sur les partenaires, les employés et les clients. Utilisées par toute l'entreprise, elles augmentent l'efficacité de l'organisation et vont au-delà d'un simple outil de vente et de marketing. Du premier contact jusqu'à la signature du contrat, le CRM enregistre chaque interaction avec un client, y compris les emails, les réunions et les présentations. Le suivi et la satisfaction des clients accélèrent ainsi la croissance de l'entreprise.

\subsection{Salesforce}

Salesforce est une plateforme de gestion de la relation client (CRM) basée sur le cloud, conçue pour centraliser et optimiser la gestion des ventes, du service client et des processus métiers. Le cœur de Salesforce, appelé Salesforce Core (Sales Cloud et Service Cloud), permet de suivre l'ensemble du cycle de vie client :

\begin{itemize}
    \item \textbf{Gestion des comptes et contacts} : centralisation des informations clients, historique des interactions, segmentation.
    \item \textbf{Opportunités et pipeline de vente} : suivi des prospects, gestion des affaires, prévisions et automatisation des processus de vente.
    \item \textbf{Service client} : gestion des cas, suivi des demandes, base de connaissances et automatisation du support.
    \item \textbf{Automatisation et personnalisation} : création de workflows, règles de validation, Process Builder, Flows pour automatiser les tâches récurrentes et garantir la qualité des données.
    \item \textbf{Extensibilité} : développement d'applications personnalisées via Apex (langage propriétaire), Lightning Components, et intégration avec des systèmes tiers via API REST/SOAP.
    \item \textbf{Sécurité et conformité} : gestion fine des accès, audit, traçabilité, gestion des consentements et conformité RGPD.
    \item \textbf{Reporting et analytique} : création de rapports dynamiques, tableaux de bord et analyses avancées pour le pilotage de l'activité.
\end{itemize}

Salesforce Core constitue ainsi la colonne vertébrale du projet AMIgo, servant de référentiel unique pour l'ensemble des données clients et des processus métiers critiques, tout en offrant une grande flexibilité d'intégration et d'automatisation.

\section{Présentation d'AMI Paris}

\begin{center}
    \includegraphics[width=0.18\textwidth]{chapters/images/ami.png} \\
    {\itshape - Logo AMI Paris -}
\end{center}

\begin{mdframed}[style=breakablegreen]
Fondée en 2011 par Alexandre Mattiussi, AMI Paris est une marque française de prêt-à-porter qui allie l'élégance parisienne à une esthétique décontractée. Le nom "AMI" — signifiant "ami" en français — reflète une vision de la mode accessible, sincère et inclusive. La marque connaît une croissance rapide à l'international grâce à sa présence multicanale : boutiques physiques (Paris, Londres, Tokyo...), e-commerce via Shopify, marketplaces, et corners dans des grands magasins. AMI Paris adopte une stratégie omnicanale forte, cherchant à offrir une expérience client unifiée entre le digital et le retail physique.
\end{mdframed}

\subsection{Écosystème Commercial}

L'écosystème commercial d'AMI Paris repose sur trois piliers principaux :
\begin{itemize}
    \item \textbf{E-commerce} : Site officiel développé sur Shopify.
    \item \textbf{Retail physique} : Boutiques gérées via le système de caisse et de gestion CEGID.
    \item \textbf{CRM \& marketing} : Précédemment basé sur Ometria, avec des capacités limitées et des problèmes d'intégration.
\end{itemize}

Chaque canal collecte des données clients qui étaient difficilement centralisées et exploitées efficacement, créant des silos d'information et limitant la personnalisation de l'expérience utilisateur.

\newpage
\section{Analyse de l'Existant}

Avant le projet, les systèmes d'information d'AMI Paris étaient cloisonnés :
\begin{itemize}
    \item Shopify hébergeait les données clients web (emails, commandes).
    \item Cegid stockait les achats en boutique et les informations transactionnelles physiques.
    \item Ometria était utilisé comme CRM et outil marketing, mais avec des capacités limitées.
\end{itemize}

\begin{mdframed}[backgroundcolor=warningorange, linewidth=0pt, innerleftmargin=10pt, innerrightmargin=10pt]
Cette segmentation empêchait une vision 360° du client, limitant la pertinence des campagnes marketing. De plus, Ometria présentait des problèmes d'intégration avec les autres systèmes et des capacités fonctionnelles limitées.
\end{mdframed}

\subsection{Systèmes d'Information Actuels}

Les SI d'AMI Paris étaient fragmentés, chaque système fonctionnant en silo et rendant difficile la centralisation et l'exploitation optimale des données clients. Contrairement à ce qui a pu être indiqué précédemment, AMI Paris ne disposait pas de Salesforce avant ce projet. L'implémentation de Salesforce a été réalisée entièrement from scratch dans le cadre du projet AMIgo.

\subsection{Limitations d'Ometria}

Avant l'adoption de Salesforce, AMI Paris utilisait Ometria comme solution CRM et marketing, avec plusieurs limitations majeures :

\begin{itemize}
    \item Gestion déficiente des intégrations entrantes et sortantes avec les autres systèmes
    \item Fonctionnalités marketing limitées et peu personnalisables
    \item Difficultés dans la réconciliation des données client entre canaux
    \item Interface utilisateur peu intuitive et workflows complexes
    \item Reporting limité et manque de capacités analytiques avancées
\end{itemize}

\subsection{Défis de l'Intégration Multicanale}

Les principaux défis rencontrés dans l'intégration multicanale incluaient :

\begin{itemize}
    \item Hétérogénéité des formats de données (codes pays/langues différents).
    \item Synchronisation en temps réel souhaitée (webhooks Shopify, batchs Cegid).
    \item Détection des doublons clients entre les sources.
    \item Absence totale d'infrastructure Salesforce nécessitant une mise en place complète
    \item Gestion des mises à jour partielles sans écraser les données de meilleure qualité.
\end{itemize}

\section{Enjeux Stratégiques du Projet}

Le projet AMIgo Client Engagement s'inscrit dans une stratégie de transformation digitale complète pour AMI Paris.

\subsection{Objectifs Business}

Les objectifs commerciaux du projet incluent :

\begin{itemize}
    \item Offrir une expérience client fluide et personnalisée, quel que soit le canal.
    \item Avoir une vision unifiée du client (historique, préférences, opt-ins).
    \item Améliorer la qualité de la donnée CRM pour renforcer les campagnes marketing.
    \item Faciliter la segmentation avancée et l'activation des audiences dans Salesforce Marketing Cloud.
\end{itemize}

\subsection{Indicateurs de Performance Clés}

Pour mesurer le succès du projet, plusieurs KPIs ont été définis :

\begin{itemize}
    \item \textbf{Taux de complétion du profil client} (données enrichies via Shopify/Cegid).
    \item \textbf{Taux de délivrabilité des emails} (liés aux opt-ins bien gérés).
    \item \textbf{Temps moyen de synchronisation des événements} (webhooks Shopify).
    \item \textbf{Taux d'unification des doublons}.
    \item \textbf{Taux d'engagement aux campagnes marketing} (avant/après projet).
\end{itemize}

\section{Méthodologie Adoptée}

\subsection{Jira, Atlassian et Confluence}
\begin{center}
    \includegraphics[width=0.18\textwidth]{chapters/images/jira_logo.png} \hspace{1cm}
    \includegraphics[width=0.18\textwidth]{chapters/images/atlassian_logo.png} \hspace{1cm}
    \includegraphics[width=0.18\textwidth]{chapters/images/Confluence_logo.png} \\
    {\itshape - Logos Jira, Atlassian et Confluence -}
\end{center}

Pour mener à bien ce projet, nous avons adopté une approche AGILE inspirée de SCRUM, reconnue pour sa souplesse et son adaptabilité. Ce choix s'est traduit par :

\begin{itemize}
    \item Des cycles courts de développement (sprints).
    \item Un feedback régulier du client AMI.
    \item Des tests incrémentaux dans des sandbox Salesforce.
\end{itemize}

Cette méthode a permis une meilleure adaptation aux contraintes fonctionnelles (marketing, CRM) et techniques (données restreintes, déclencheurs Apex).

\subsection{Choix de la Méthodologie}

Nous avons utilisé Jira, Confluence et la suite Atlassian comme plateformes centralisées pour la gestion de projet. Jira a permis de planifier, suivre et gérer les tâches, tandis que Confluence a servi de base documentaire collaborative pour centraliser les spécifications, les comptes-rendus et les documents de référence. Cette combinaison a facilité la collaboration, l'organisation des tâches et le suivi des livrables. L'approche AGILE, combinée à l'utilisation de Jira, Confluence et Atlassian, a permis d'assurer une communication fluide et une réactivité optimale face aux imprévus.

\subsection{Processus de Gestion des Tâches}

Le processus de gestion des tâches s'est articulé autour des étapes suivantes :
\begin{enumerate}
    \item \textbf{Création et affectation des tâches} : Les tâches sont créées dans Jira, détaillées et assignées aux membres de l'équipe selon leurs compétences et disponibilités.
    \item \textbf{Travail sur la tâche} : Chaque membre travaille sur les tâches qui lui sont attribuées, en documentant l'avancement et les points importants dans Confluence si nécessaire.
    \item \textbf{Revue, confirmation et validation} : Une fois la tâche terminée, elle est soumise à une revue (peer review ou validation par le responsable). Les retours sont intégrés si besoin.
    \item \textbf{Clôture de la tâche} : Après validation, la tâche est marquée comme terminée dans Jira. L'information est synchronisée dans Confluence pour assurer la traçabilité et la documentation du projet.
\end{enumerate}
Ce processus garantit la transparence, la traçabilité et la qualité des livrables tout au long du projet.

\subsection{Phases du Projet}

\begin{mdframed}[backgroundcolor=lightblue, linewidth=0pt, innerleftmargin=10pt, innerrightmargin=10pt]
Le projet s'est articulé autour de plusieurs phases clés :
\begin{itemize}
    \item \textbf{Phase d'analyse} : Audit de l'existant, relevé des besoins métiers, spécifications fonctionnelles et techniques.
    \item \textbf{Phase de développement} : Création des web services Apex pour la réception des données Shopify, mise en place des handlers Apex (comme AccountTriggerHandler), configuration des Custom Metadata Types (ex. : Country → Language mapping).
    \item \textbf{Phase de test} : Écriture de classes de test avec une couverture supérieure à 90\%, vérification des flux de données en sandbox via Postman et simulations de webhooks.
    \item \textbf{Phase de mise en production} : Déploiement contrôlé via Change Sets, monitoring et ajustements post go-live.
\end{itemize}
\end{mdframed}

\subsection{Timeline des Sprints}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{chapters/images/timeline_board.png}
    \caption{Planification des sprints et lots fonctionnels du projet AMIgo}
    \label{fig:sprint-timeline}
\end{figure}

\section{Conclusion}

\begin{mdframed}[style=breakablegreen]
Le projet mené pour AMI Paris s'inscrit dans une logique de transformation digitale complète. En intégrant Shopify et Cegid à Salesforce, la marque se dote d'un CRM centralisé, capable de porter une stratégie marketing fondée sur la donnée, la personnalisation, et la cohérence omnicanale. Cette mission nous a permis d'approfondir nos compétences en intégration de systèmes, en développement Apex, et en gestion de projet Agile dans un contexte professionnel exigeant.
\end{mdframed}

% Suggestion : pour une police moderne, ajouter dans le préambule : \usepackage{lmodern}
